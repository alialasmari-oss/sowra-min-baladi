...
function setSort(m){sortMode=m;$('tabTop').classList.toggle('on',m==='top');$('tabNew').classList.toggle('on',m==='new');render()}
...
function render(){
  ...
  $('totalPill').textContent=`${photos.length} صورة · م4`;
  const feed=$('feed');
...
async function openSheet(id){
  ...
  $('sLoc').innerHTML=`📍 ${esc(p.region)} · ${esc(p.city)}${p.village?' · '+esc(p.village):''} — عدسة ${esc(p.photographer)}`
